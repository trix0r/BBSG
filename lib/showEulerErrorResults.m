function [data, latex] = showResults(ids, mode, suppressOutput, policyName)
...
if ~exist('policyName', 'var'); usePolicy = false; else; usePolicy = true; end
...
switch mode
    ...
    case 'complexity'
       ...
       latexHeader = '';
    case 'errors'
        header = {'ID', 'd', 'Grid', 'Model', 'PL', 'PTol', '#PPts', '#PIns', 'PTime', ...
                  'GL2', 'GLInf', 'EL2', 'ELInf'};
        formatString = '%4s %1s %4s %7s | %2s %4s %5s %5s %6s | %6s %6s | %6s %6s\n';
        strLine = sprintf(['--------------------+---------------+' ...
                           '----------------------------+---------------\n']);
       latexHeader = '';
    otherwise
...
for i = 1:numel(resultsDirs)
    ...
    try
        ...
        elseif strcmp(mode, 'errors')
            [properties, problem, ~, ~, ~, errors, ~, envelopeErrors] = loadResult(id, ...
                    {'properties', 'problem', 'errors', 'envelopeErrors'});
        else
    ...
    if usePolicy; policyIdx = strcmpi(policyName, problem.policyNames); else; policyIdx = 1; end
    if ~isempty(errors)
        err = struct2struct(errors);
        errorsL2   = nanmean(err.L2(:, policyIdx));
        errorsLInf = nanmean(err.Max(:, policyIdx));
    else
        errorsL2 = NaN;
        errorsLInf = NaN;
    end
    ...
    if strcmp(mode, 'errors')
        if ~isempty(envelopeErrors)
            try
                envelopeErr = struct2struct(envelopeErrors);
                envelopeErrorsL2   = nanmean(envelopeErr.L2(:, policyIdx));
                envelopeErrorsLInf = nanmean(envelopeErr.Max(:, policyIdx));
            catch
                % if there is no envelope error for policyIdx
                envelopeErrorsL2 = NaN;
                envelopeErrorsLInf = NaN;
            end
        else
            envelopeErrorsL2 = NaN;
            envelopeErrorsLInf = NaN;
    ...
    if strcmp(mode, 'errors')
        model = strrep(properties.problem, 'Problem', '');
    end
        ...
        case 'default'
            if printFlag
                fprintf(formatString, ...
                        sprintf('%04u', id), ...
                        formatFloat(problem.numberOfStates, 0), ...
                        gridType, ...
        ...
        case 'errors'
            if printFlag
                fprintf(formatString, ...
                        ...
                        formatScientific(errorsL2, 1), ...
                        formatScientific(errorsLInf, 1), ...
                        formatScientific(envelopeErrorsL2, 1), ...
                        formatScientific(envelopeErrorsLInf, 1));
            end
            tblData = {id, problem.numberOfStates, gridType, model, parameters.policyBaseLevel, ...
                       policyRefineTolerance, numberOfPointsPolicy, ...
                       numberOfInsertedPointsPolicy, timePolicy, errorsL2, errorsLInf, ...
                       envelopeErrorsL2, envelopeErrorsLInf};
    end
