...
for t = times
    C(:, t) = self.computeConsumptionForPolicy([state.W(:, t) state.L(:, t)], discreteState, t, ...
...
plot(age, mean(state.L(:, times)) * 10, 'y', 'LineWidth', 2);
...
legend('cash on hand', 'annuities', 'consumption', 'stocks', 'bonds', 'income', 'Location', 'best')
