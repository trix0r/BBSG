function errors = computeStateSpaceEnvelopeEulerErrors(self, solution, interpPolicy, times)
...
parfor r = 1:numel(times)
    ...
    for q = 1:self.numberOfDiscreteStates %#ok<PFBNS>
        ...
        for i = 1:size(self.EulerError(t).points, 1)
            err(i, :) = self.getEnvelopeEulerErrors(self.EulerError(t).points(i, :), ...
                                                    discreteStateName, t, solution, interpPolicy);
        end
...
try
    save([self.Env.outdir filesep 'envelope_euler_errors.mat'], 'self', ...
         'interpPolicy', 'times', 'errors');
catch
    warning('Couldn''t save envelope_euler_errors.mat.');
end
